...
def connect_to_server():
    ...
    while True:
        with lock:
            if is_busy:
                print("[WAITING] Busy with another task. Retrying in 1 seconds...")
                time.sleep(1)
                continue
            
            try:
                is_busy = True  # ตั้งสถานะเป็น busy
                client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                client.connect((SERVER_IP, SERVER_PORT))
                print(f"[CONNECTED] to Server {SERVER_IP}:{SERVER_PORT}")
                is_busy = False  # รีเซ็ตสถานะเมื่อเชื่อมต่อสำเร็จ
                return client
            except:
                is_busy = False  # รีเซ็ตสถานะเมื่อเชื่อมต่อไม่สำเร็จ
...
while True:
    try:
        ...
        with lock:
            ...
            print("-----------------------------------")
            is_busy = False  # รีเซ็ตสถานะเมื่อส่งข้อมูลเสร็จ
    except:
        server_socket = connect_to_server()